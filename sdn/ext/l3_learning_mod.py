...
import time
from packet_info_extractor import handle_IP_packet
...
class l3_switch (EventMixin):
  ...
  def block_flow (self, event, packet_info):
    """
    Block flow that matches the passed packet info's 
    srcip and dstip by installing rules to drop
    drops both IP and ARP packets
    """
    log.debug("Installing dropping rule for IP")
    msg = of.ofp_flow_mod(command= of.OFPFC_ADD,
                          idle_timeout= of.OFP_FLOW_PERMANENT,
                          hard_timeout= of.OFP_FLOW_PERMANENT,
                          buffer_id=event.ofp.buffer_id)
    msg.match.dl_type = ethernet.IP_TYPE
    msg.match.nw_src = packet_info["srcip"]
    msg.match.nw_dst = packet_info["dstip"]
    msg.actions = []
    event.connection.send(msg.pack())

    log.debug("Installing dropping rule for ARP")
    msg.match.dl_type = ethernet.ARP_TYPE
    event.connection.send(msg.pack())
  
  def redirect_flow (self, event, packet_info, redirect_dst):
    """
    Redirect flow that macthes the passed packet info's 
    srcip and dstip to """
    log.debug("Installing redirection rule for IP")
    msg = of.ofp_flow_mod(command= of.OFPFC_ADD,
                          idle_timeout= of.OFP_FLOW_PERMANENT,
                          hard_timeout= of.OFP_FLOW_PERMANENT,
                          buffer_id=event.ofp.buffer_id)
    ...
    msg.match.nw_src = packet_info["srcip"]
    msg.match.nw_dst = packet_info["dstip"]
    msg.actions.append(of.ofp_action_nw_addr.set_dst(redirect_dst))
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      packet_info = handle_IP_packet(packet.next)
      # lets say we do some shit and find this type of packet should be dropped
      if packet_info["srcip"] == IPAddr("10.0.0.1") and packet_info["dstip"] == IPAddr("10.0.0.2"):
        print(packet_info)
        # self.block_flow(event, packet_info)
        self.redirect_flow(event, packet_info, IPAddr("10.0.0.3"))
        return
